...
pub struct IndexSet<T, S = RandomState, Idx = usize> {
    map: IndexMap<T, (), S, Idx>,
}
#[cfg(not(has_std))]
pub struct IndexSet<T, S, Idx = usize> {
    map: IndexMap<T, (), S, Idx>,
}

impl<T, S, Idx> Clone for IndexSet<T, S, Idx>
where
    T: Clone,
    S: Clone,
    Idx: Index,
...
impl<T, S, Idx: Index> Entries for IndexSet<T, S, Idx> {
    type Entry = Bucket<T>;
...
impl<T, S, Idx> fmt::Debug for IndexSet<T, S, Idx>
where
    T: fmt::Debug + Hash + Eq,
    S: BuildHasher,
    Idx: Index + fmt::Debug,
...
impl<T, S, Idx: Index> IndexSet<T, S, Idx> {
    /// Create a new set with capacity for `n` elements.
    /// (Does not allocate if `n` is zero.)
    ///
    /// Computes in **O(n)** time.
    ...
    Idx: Index,
{
    ...
    ///
    /// Computes in **O(1)** time (amortized average).
    pub fn insert_full(&mut self, value: T) -> (Idx, bool) {
        use super::map::Entry::*;
    ...
    pub fn difference<'a, S2>(
        &'a self,
        other: &'a IndexSet<T, S2, Idx>,
    ) -> Difference<'a, T, S2, Idx>
    where
        S2: BuildHasher,
    {
        Difference {
    ...
    pub fn symmetric_difference<'a, S2>(
        &'a self,
        other: &'a IndexSet<T, S2, Idx>,
    ) -> SymmetricDifference<'a, T, S, S2, Idx>
    where
        S2: BuildHasher,
    {
        SymmetricDifference {
    ...
    pub fn union<'a, S2>(&'a self, other: &'a IndexSet<T, S2, Idx>) -> Union<'a, T, S, Idx>
    where
        S2: BuildHasher,
    {
        Union {
    ...
    pub fn get_full<Q: ?Sized>(&self, value: &Q) -> Option<(Idx, &T)>
    where
        Q: Hash + Equivalent<T>,
    {
    ...
    pub fn get_index_of<Q: ?Sized>(&self, value: &Q) -> Option<Idx>
    where
        Q: Hash + Equivalent<T>,
    {
    ...
    ///
    /// Return `None` if `value` was not in the set.
    pub fn swap_remove_full<Q: ?Sized>(&mut self, value: &Q) -> Option<(Idx, T)>
    where
        Q: Hash + Equivalent<T>,
    {
    ...
    ///
    /// Return `None` if `value` was not in the set.
    pub fn shift_remove_full<Q: ?Sized>(&mut self, value: &Q) -> Option<(Idx, T)>
    where
        Q: Hash + Equivalent<T>,
    {
...
impl<T, S, Idx: Index> IndexSet<T, S, Idx> {
    ...
    ///
    /// Computes in **O(1)** time (average).
    pub fn swap_remove_index(&mut self, index: Idx) -> Option<T> {
...
impl<'a, T, S, Idx> IntoIterator for &'a IndexSet<T, S, Idx>
where
    T: Hash + Eq,
    S: BuildHasher,
    Idx: Index,
...
impl<T, S, Idx> IntoIterator for IndexSet<T, S, Idx>
where
    T: Hash + Eq,
    S: BuildHasher,
    Idx: Index,
...
impl<T, S, Idx> FromIterator<T> for IndexSet<T, S, Idx>
where
    T: Hash + Eq,
...
impl<T, S, Idx> Extend<T> for IndexSet<T, S, Idx>
where
    T: Hash + Eq,
    S: BuildHasher,
    Idx: Index,
...
impl<'a, T, S, Idx> Extend<&'a T> for IndexSet<T, S, Idx>
where
    T: Hash + Eq + Copy + 'a,
    S: BuildHasher,
    Idx: Index,
...
impl<T, S, Idx> Default for IndexSet<T, S, Idx>
where
    S: BuildHasher + Default,
    Idx: Index,
{
...
impl<T, S1, S2, Idx> PartialEq<IndexSet<T, S2, Idx>> for IndexSet<T, S1, Idx>
where
    T: Hash + Eq,
    S1: BuildHasher,
    S2: BuildHasher,
    Idx: Index,
{
    fn eq(&self, other: &IndexSet<T, S2, Idx>) -> bool {
        self.len() == other.len() && self.is_subset(other)
    }
}

impl<T, S, Idx> Eq for IndexSet<T, S, Idx>
where
    T: Eq + Hash,
    S: BuildHasher,
    Idx: Index,
    ...
    Idx: Index,
{
    ...
    pub fn is_disjoint<S2>(&self, other: &IndexSet<T, S2, Idx>) -> bool
    where
        S2: BuildHasher,
    {
    ...
    pub fn is_subset<S2>(&self, other: &IndexSet<T, S2, Idx>) -> bool
    where
        S2: BuildHasher,
    {
    ...
    pub fn is_superset<S2>(&self, other: &IndexSet<T, S2, Idx>) -> bool
    where
        S2: BuildHasher,
    {
...
impl<'a, T, S, Idx> Iterator for Difference<'a, T, S, Idx>
where
    T: Eq + Hash,
    S: BuildHasher,
    Idx: Index,
...
impl<'a, T, S, Idx> DoubleEndedIterator for Difference<'a, T, S, Idx>
where
    T: Eq + Hash,
    S: BuildHasher,
    Idx: Index,
...
impl<'a, T, S, Idx> Clone for Difference<'a, T, S, Idx> {
    fn clone(&self) -> Self {
        Difference {
            iter: self.iter.clone(),
            ..*self
...
impl<'a, T, S, Idx> fmt::Debug for Difference<'a, T, S, Idx>
where
    T: fmt::Debug + Eq + Hash,
    S: BuildHasher,
    Idx: Index,
...
impl<'a, T, S, Idx> Iterator for Intersection<'a, T, S, Idx>
where
    T: Eq + Hash,
    S: BuildHasher,
    Idx: Index,
...
impl<'a, T, S, Idx> DoubleEndedIterator for Intersection<'a, T, S, Idx>
where
    T: Eq + Hash,
    S: BuildHasher,
    Idx: Index,
...
impl<'a, T, S, Idx> Clone for Intersection<'a, T, S, Idx> {
    fn clone(&self) -> Self {
        Intersection {
            iter: self.iter.clone(),
            ..*self
...
impl<'a, T, S, Idx> fmt::Debug for Intersection<'a, T, S, Idx>
where
    T: fmt::Debug + Eq + Hash,
    S: BuildHasher,
    Idx: Index,
...
impl<'a, T, S1, S2, Idx> Iterator for SymmetricDifference<'a, T, S1, S2, Idx>
where
    T: Eq + Hash,
    S1: BuildHasher,
    S2: BuildHasher,
...
impl<'a, T, S1, S2, Idx> DoubleEndedIterator for SymmetricDifference<'a, T, S1, S2, Idx>
where
    T: Eq + Hash,
    S1: BuildHasher,
    S2: BuildHasher,
...
impl<'a, T, S1, S2, Idx> Clone for SymmetricDifference<'a, T, S1, S2, Idx> {
    fn clone(&self) -> Self {
        SymmetricDifference {
            iter: self.iter.clone(),
        }
...
impl<'a, T, S1, S2, Idx> fmt::Debug for SymmetricDifference<'a, T, S1, S2, Idx>
where
    T: fmt::Debug + Eq + Hash,
    S1: BuildHasher,
    S2: BuildHasher,
...
impl<'a, T, S, Idx> Iterator for Union<'a, T, S, Idx>
where
    T: Eq + Hash,
    S: BuildHasher,
    Idx: Index,
...
impl<'a, T, S, Idx> DoubleEndedIterator for Union<'a, T, S, Idx>
where
    T: Eq + Hash,
    S: BuildHasher,
    Idx: Index,
...
impl<'a, T, S, Idx> Clone for Union<'a, T, S, Idx> {
    fn clone(&self) -> Self {
        Union {
            iter: self.iter.clone(),
        }
...
impl<'a, T, S, Idx> fmt::Debug for Union<'a, T, S, Idx>
where
    T: fmt::Debug + Eq + Hash,
    S: BuildHasher,
    Idx: Index,
...
impl<'a, 'b, T, S1, S2, Idx> BitAnd<&'b IndexSet<T, S2, Idx>> for &'a IndexSet<T, S1, Idx>
where
    T: Eq + Hash + Clone,
    ...
    Idx: Index,
{
    type Output = IndexSet<T, S1, Idx>;

    /// Returns the set intersection, cloned into a new set.
    ///
    /// Values are collected in the same order that they appear in `self`.
    fn bitand(self, other: &'b IndexSet<T, S2, Idx>) -> Self::Output {
        self.intersection(other).cloned().collect()
    }
}

impl<'a, 'b, T, S1, S2, Idx> BitOr<&'b IndexSet<T, S2, Idx>> for &'a IndexSet<T, S1, Idx>
where
    T: Eq + Hash + Clone,
    S1: BuildHasher + Default,
    S2: BuildHasher,
    Idx: Index,
{
    ...
    /// values that are unique to `other` in their original order.
    fn bitor(self, other: &'b IndexSet<T, S2, Idx>) -> Self::Output {
        self.union(other).cloned().collect()
    }
}

impl<'a, 'b, T, S1, S2, Idx> BitXor<&'b IndexSet<T, S2, Idx>> for &'a IndexSet<T, S1, Idx>
where
    T: Eq + Hash + Clone,
    S1: BuildHasher + Default,
    S2: BuildHasher,
    Idx: Index,
{
    ...
    /// Values from `self` are collected in their original order, followed by
    /// values from `other` in their original order.
    fn bitxor(self, other: &'b IndexSet<T, S2, Idx>) -> Self::Output {
        self.symmetric_difference(other).cloned().collect()
    }
}

impl<'a, 'b, T, S1, S2, Idx> Sub<&'b IndexSet<T, S2, Idx>> for &'a IndexSet<T, S1, Idx>
where
    T: Eq + Hash + Clone,
    S1: BuildHasher + Default,
    S2: BuildHasher,
    Idx: Index,
{
    ...
    ///
    /// Values are collected in the same order that they appear in `self`.
    fn sub(self, other: &'b IndexSet<T, S2, Idx>) -> Self::Output {
