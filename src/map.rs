...
pub use map_core::{Entry, Index, OccupiedEntry, VacantEntry};
...
impl<K, V, S, Idx: Index> Entries for IndexMap<K, V, S, Idx> {
...
impl<K, V, S, Idx: Index> IndexMap<K, V, S, Idx> {
    /// Create a new map with capacity for `n` key-value pairs. (Does not
    /// allocate if `n` is zero.)
    ///
    /// Computes in **O(n)** time.
    ...
    Idx: Index,
{
    ...
    ///
    /// Computes in **O(1)** time (amortized average).
    pub fn entry(&mut self, key: K) -> Entry<K, V, Idx> {
        let hash = self.hash(&key);
    ...
    pub(crate) fn get_full_mut2_impl<Q: ?Sized>(&mut self, key: &Q) -> Option<(Idx, &mut K, &mut V)>
    where
        Q: Hash + Equivalent<K>,
    {
        if let Some(i) = self.get_index_of(key) {
...
impl<K, V, S, Idx: Index> IndexMap<K, V, S, Idx> {
    ...
    ///
    /// Computes in **O(1)** time.
    pub fn get_index_mut(&mut self, index: Idx) -> Option<(&mut K, &mut V)> {
        self.as_entries_mut()
...
impl<'a, K, V, Q, S, Idx> ops::Index<&'a Q> for IndexMap<K, V, S, Idx>
where
...
impl<'a, K, V, Q, S, Idx> ops::IndexMut<&'a Q> for IndexMap<K, V, S, Idx>
where
    Q: ?Sized + Hash + Equivalent<K>,
    K: Hash + Eq,
    S: BuildHasher,
